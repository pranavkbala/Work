...
*/
 SELECT
         ...
         STG.EXCHANGE_ORDER_ID,                      /* Added as part SEPIK changes */
		     COALESCE(LO.LOCATION_KEY, -1) AS LOCATION_KEY,
		     STG.LOCATION_ID,
         ...
         STG.MATCH_METHOD_CD,
         case
           when STG.ORDER_HEADER_KEY <> 0 then  STG.MODIFIEDDATETIME
           else STG.CREATEDDATETIME
         end AS FIRST_EFFECTIVE_TS,
     ...
     STG.SUBORDERS_CNT,  /* SUTLEJ CHANGE */
         ...
         STG.INSERT_TS,
	     STG.UPDATE_TS
   FROM RJ_OH_VIEW2 STG
   LEFT OUTER JOIN L2_ANALYTICS.LOCATION LO
        ON STG.LOCATION_ID = LO.LOCATION_ID
        AND LO.SOURCE_SYSTEM = 'RJDAX'
        AND LO.LAST_EFFECTIVE_TS = '9999-12-31T23:59:59.000+0000'
