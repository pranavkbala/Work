...
**  03    	06/08/2021  SEPIK  Added  columns as part of
								  adding to OH from Sterling OH
**  04      07/23/2021  SUT-160  Adding columns as part of
								  adding to OH from Sterling OH
...
** Purpose      	: ingest data from stage to Delta table

Source : L2_STAGE.RJ_ORDER_HEADER_STG
Target : L2_ANALYTICS_TABLES.ORDER_HEADER

******************************************************************
******************************************************************
*/

         MERGE INTO L2_ANALYTICS_TABLES.ORDER_HEADER TARGET USING
        (
           SELECT STG_1.ORDER_HEADER_KEY AS MERGE_KEY, STG_1.* FROM L2_STAGE.RJ_ORDER_HEADER_STG STG_1
	         UNION
	         SELECT NULL AS MERGE_KEY, STG_2.* FROM L2_STAGE.RJ_ORDER_HEADER_STG STG_2
        ) STG ON STG.ORDER_DT = TARGET.ORDER_DT
              AND TARGET.SOURCE_SYSTEM = 'RJ'
              AND TARGET.ORDER_HEADER_KEY = STG.MERGE_KEY
              AND STG.LAST_EFFECTIVE_TS = '9999-12-31T23:59:59.000+0000'
